...
- **`git clone`:** Es un comando que permite copiar un repositorio remoto de GitHub a nuestro equipo local. Al clonarlo podemos trabajar con sus archivos, ramas e historial desde nuestra computadora.

- **Fork:** Es una copia de un repositorio que se crea dentro de nuestra propia cuenta de GitHub. Esto permite trabajar sobre el proyecto sin modificar directamente el repositorio original.

- **`git pull`:** Es un comando que permite traer los cambios más recientes del repositorio remoto y actualizar nuestro repositorio local.
...
Primero ingresé al repositorio original proporcionado por el docente. Luego seleccioné la opción **Fork** en GitHub y elegí mi cuenta personal para crear una copia del repositorio.

De esta manera obtuve una copia del proyecto dentro de mi propia cuenta de GitHub, sin modificar directamente el repositorio original.
...
Después de crear el fork, ingresé al repositorio que se creó en mi cuenta personal de GitHub.

Seleccioné la opción **Code**, elegí **HTTPS** y copié la dirección del repositorio.
...
Después ingresé a la carpeta del repositorio con:

```bash
cd EVALUACION_1P
```

Con esto se creó una copia local de mi fork en la computadora.
...
Este comando mostró las direcciones utilizadas para realizar operaciones de `fetch` y `push`.

Al observar el resultado se verificó que la dirección de `origin` correspondía al repositorio ubicado en mi cuenta personal de GitHub. De esta manera se comprobó que estaba trabajando sobre mi fork y no directamente sobre el repositorio original.

#### Evidencia del repositorio remoto

![Evidencia del Remote](images/Pregunta_1/remote.png)
...
Se realizó correctamente el fork del repositorio original, posteriormente se clonó la copia personal en el equipo local y mediante el comando `git remote -v` se verificó que el repositorio local se encontraba conectado al fork de mi cuenta personal de GitHub.

```

Ahora sí, seguimos **paso por paso para sacar `fork.png`, `clone.png` y `remote.png`**.
```
